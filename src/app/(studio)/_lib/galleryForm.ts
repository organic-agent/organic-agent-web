/**
 * 작가 — 갤러리 폼 유틸
 * 위치: src/app/(studio)/_lib/galleryForm.ts
 *
 * 갤러리 생성/수정에서 공유하는 입력값 형태와 검증 규칙을 모은다.
 * 필드는 생성 계약(POST /api/v1/galleries)이 받는 네 값과 1:1이다 —
 * 컨셉 개수·메모는 기획에서 제거됐다(WES-21 결정). 보정 횟수는 2026-09-12 추가(서버 maxRetouchRoundCount).
 *
 * 주요 책임:
 * - 갤러리 폼 기본값 생성
 * - 저장 가능 여부 검증
...
export type GalleryFormValues = {
  ...
  target: string;
  /** 계약 보정 횟수(1~20). 빈 문자열이면 제한 없음(서버 null). */
  rounds: string;
};

/** 보정 횟수 입력이 유효한지 — 비었거나 1~20 정수(ChangeRoundsModal과 같은 범위) */
export function isRoundsValid(rounds: string) {
  if (rounds.trim() === "") return true;
...
export function createDefaultGalleryForm(): GalleryFormValues {
  ...
  return { name: "", dueDate: "", target: "", rounds: "" };
}
...
export function isGalleryFormValid(values: GalleryFormValues) {
  ...
  return (
    values.name.trim().length > 0 && !isPastDueDate(values.dueDate) && targetValid && isRoundsValid(values.rounds)
  );
}
