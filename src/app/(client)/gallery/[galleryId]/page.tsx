...
import { AlbumTreeSection } from "@/app/(studio)/studio/gallery/[galleryId]/_components/AlbumTreeSection";
import { ClusterStackCell } from "@/app/(studio)/studio/gallery/[galleryId]/_components/ClusterStackCell";
import { DeleteFolderModal } from "@/app/(studio)/studio/gallery/[galleryId]/_components/DeleteFolderModal";
import { PhotoContextMenu } from "@/app/(studio)/studio/gallery/[galleryId]/_components/PhotoContextMenu";
import { SaveAlbumModal } from "@/app/(studio)/studio/gallery/[galleryId]/_components/SaveAlbumModal";
import { SelectionActionBar } from "@/app/(studio)/studio/gallery/[galleryId]/_components/SelectionActionBar";
import { useClusterPreview } from "@/app/(studio)/studio/gallery/[galleryId]/_lib/useClusterPreview";
import { useFolderGroups } from "@/app/(studio)/studio/gallery/[galleryId]/_lib/useFolderGroups";
import { useFolderPhotos } from "@/app/(studio)/studio/gallery/[galleryId]/_lib/useFolderPhotos";
import {
  deadlineLabel,
  isDeadlinePassed,
  useInvitedGallery,
...
export default function CoupleGalleryWorkspacePage() {
  ...
  // 주소 /gallery/[galleryId] — 어느 갤러리인지는 주소가 정한다. 멤버가 아니면 notFound.
  const params = useParams<{ galleryId: string }>();
  const { result: invited, reload: reloadInvited } = useInvitedGallery(params.galleryId);
  ...
  if (!gallery) {
    const state =
      invited === null
        ...
        : invited.kind === "notFound"
          ? {
              title: "갤러리를 찾을 수 없어요",
              desc: "초대받은 갤러리가 아니거나 주소가 잘못됐어요. 받은 초대 링크를 다시 열어 주세요.",
              retry: false,
            }
          : {
