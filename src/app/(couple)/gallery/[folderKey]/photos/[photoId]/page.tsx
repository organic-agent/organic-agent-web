...
/**
 * 부부 — 폴더 상세 (사진 셀렉)
 * 위치: src/app/(couple)/gallery/[folderKey]/photos/[photoId]/page.tsx
 *
...
export default function FolderDetailPage() {
  const { folderKey, photoId } = useParams<{
    folderKey: string;
    photoId: string;
  }>();
  ...
  const [current, setCurrent] = useState(() => {
    const index = folder?.photos.findIndex((item) => item.id === Number(photoId));
    return index !== undefined && index >= 0 ? index : 0;
  });
  const [photoFilter, setPhotoFilter] = useState<PhotoFilter>("all");
  const [comparisonPicking, setComparisonPicking] = useState(false);
  // 우측 정보 패널 접기/펴기
  ...
  function toggleSelected() {
    ...
    if (isSelected) {
      ...
      const wasAutoGood =
        currentDecision === "good" && isAutoGood(photo.id);
      removeFromSelected(photo.id);
      clearAutoGood(photo.id);
      if (wasAutoGood) clearCompareTag(photo.id);
      return;
    }
    if (currentDecision === "remove") {
      setPendingSelectionConflict({
        kind: "select-excluded",
        photoId: photo.id,
      });
      return;
    }
    const result = addToSelected([photo.id]);
    if (result.added.includes(photo.id) && !currentDecision) {
  ...
  function selectFilmstripPhoto(index: number) {
    ...
    const comparisonPhoto = photos[index];
    if (!photo || !comparisonPhoto || comparisonPhoto.id === photo.id) return;
    const query = new URLSearchParams({
      left: String(photo.id),
      right: String(comparisonPhoto.id),
      filter: photoFilter,
    });
    router.push(
      `/gallery/${encodeURIComponent(decodedKey)}/compare?${query.toString()}`,
    );
  }
  ...
  function confirmSelectionConflict() {
    ...
    const { kind, photoId: pendingPhotoId } = pendingSelectionConflict;
    setPendingSelectionConflict(null);
    setSelectionNotice("");

    if (kind === "exclude-selected") {
      removeFromSelected(pendingPhotoId);
      setCompareTag(pendingPhotoId, "remove");
      return;
    }

    const result = addToSelected([pendingPhotoId]);
    if (result.added.includes(pendingPhotoId)) {
      setCompareTag(pendingPhotoId, "good");
      markAutoGood(pendingPhotoId);
    }
    ...
    <div className="h-dvh bg-white flex overflow-hidden">
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        ...
        <header className="h-16 shrink-0 border-b border-line flex items-center gap-4 px-6">
          <Link
            href={`/gallery/${encodeURIComponent(folder.key)}`}
          ...
          <button
            type="button"
            onClick={() => setComparisonPicking((current) => !current)}
            disabled={filteredTotal < 2}
            title={
              filteredTotal < 2
                ? "비교하려면 현재 필터에 사진이 2장 이상 필요해요."
                : undefined
            }
            className={`ml-auto h-9 px-4 rounded-pill text-[12.5px] font-medium shrink-0 transition-colors ${
              comparisonPicking
                ? "border border-ink bg-ink text-on-ink"
                : "border border-line bg-white text-ink-2 hover:border-ink-3 hover:text-ink"
            } disabled:opacity-40 disabled:pointer-events-none`}
          >
            {comparisonPicking ? "비교 선택 취소" : "비교 셀렉하기"}
          </button>
        </header>

        {comparisonPicking && photo && (
          <div className="h-10 shrink-0 border-b border-line bg-paper px-6 flex items-center justify-between gap-4">
            <p className="text-[12px] text-ink-2 truncate">
              <span className="font-medium text-ink">
                사진 #{String(photo.id).padStart(3, "0")}
              </span>
              을 A로 선택했어요. 아래에서 비교할 사진을 한 장 더 골라주세요.
            </p>
            <span className="text-[11px] text-ink-3 shrink-0">
              {filteredTotal - 1}장 선택 가능
            </span>
          </div>
        )}
        ...
        <PhotoFilmstrip
          ...
          onSelect={selectFilmstripPhoto}
          comparisonPhotoId={comparisonPicking ? photo?.id : undefined}
        />
